  ...
  try {
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: `Forex-Analyst: Analysiere ${pair} (${now}). NUR JSON ohne Backticks: {"signal":"BUY oder SELL oder NEUTRAL","entry":"1.1620","sl":"1.1580","tp":"1.1700","confidence":7,"reason":"2-3 Sätze Deutsch"}` }]
      })
    });
    const data = await response.json();
    if (data.error) return res.status(400).json({ error: data.error.message });
